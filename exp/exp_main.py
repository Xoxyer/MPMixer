...
from models import Informer, Autoformer, Transformer, DLinear, Linear, NLinear, PatchTST, MPMixer
...
class Exp_Main(Exp_Basic):
    ...
    def test(self, setting, test=0):
        ...
        if test:
            print('loading model')
            a = os.path.join('./checkpoints/' + setting, 'checkpoint.pth')
        ...
        if not os.path.exists(folder_path): #保存真实值与预测值图示
        ...
        f = open("result.txt", 'a')
        f.write(setting + "  \n")
        f.write('mse:{}, mae:{}, rse:{}, rmse:{}, mspe:{}'.format(mse, mae, rse, rmse, mape))
